...
namespace MUDGame
{
    ...
    void Player::SendString(const std::string& p_string)
    {
        ...
        if (Conn() == 0)
        {
            printf("[WARNING] TRYING TO SEND STRING TO PLAYER BUT PLAYER IS NOT CONNECTED \n");
            return;
        }

        // send the string, newline included.
        std::string send_string = p_string + "\n";
        Conn()->Send(send_string.c_str(), send_string.size());
    ...
    istream& operator>>(istream& p_stream, Player& p)
    {
        ...
        p_stream >> temp; extract<sint64>(p_stream, p.m_nextattacktime);
        p_stream >> p.m_baseattributes;
